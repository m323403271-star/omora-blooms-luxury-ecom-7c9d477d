...
import {
  ...
  Truck, Sparkles, MapPin, User, Phone, Home, CheckCircle,
} from "lucide-react";
  ...
  head: ({ params }) => {
    ...
    return {
      meta: [
        ...
        { name: "description", content: `Order ${title} — luxury handmade bouquet with same-day delivery in Bengaluru.` },
      ],
    };
  },
  loader: ({ context, params }) => {
    // Pre-load all variants into memory for generated slugs
    getAllVariants();
    // Also ensure products are cached
...
async function loadRazorpay(): Promise<boolean> {
  ...
  if (existing) return new Promise((res) => { existing.onload = () => res(true); existing.onerror = () => res(false); });
  ...
  return (
    <div>
      <label className="flex items-center gap-1.5 text-[11px] tracking-[0.18em] uppercase text-[color:var(--gold)] mb-1.5">
        <Icon className="h-3.5 w-3.5" /> {label}{required && <span className="text-[color:var(--destructive)]">*</span>}
      </label>
      <input
        type={type}
        value={value}
...
function BuyPage() {
  ...
  // Look up variant — try pre-defined first, then try to find it in all variants
  ...
  const isFormValid = name.trim() && phone.trim().length >= 10 && address.trim() && city.trim() && pincode.trim().length === 6;
  ...
  async function handleRazorpay() {
    ...
    try {
      ...
      if (!ok) { toast.error("Could not load payment gateway. Please try WhatsApp."); return; }
        ...
        handler: async (response: unknown) => {
          ...
          try {
            const verifyRes = await fetch("/api/razorpay/verify", {
              ...
              body: JSON.stringify({
                ...r,
                items: [{ id: variant.slug, quantity: 1 }],
              }),
            });
    ...
    return (
      <div className="container-luxe py-24 text-center">
        ...
        <p className="text-[color:var(--muted-foreground)] max-w-md mx-auto">
          Thank you, {name}. Our concierge will call you shortly to confirm delivery details for your <strong className="text-white">{variant.name}</strong>.
        </p>
  ...
  return (
    <div>
      ...
      <div className="container-luxe pt-8 flex items-center gap-1.5 text-[11px] tracking-[0.18em] uppercase text-[color:var(--muted-foreground)]">
        ...
        <Link to="/varieties/$slug" params={{ slug: variant.parentSlug }} className="hover:text-[color:var(--gold)] transition-colors">
          {variant.parentName}
        </Link>
        <ChevronRight className="h-3 w-3" />
            ...
            <div className="glass-card rounded-2xl p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Pincode" icon={MapPin} value={pincode} onChange={setPincode} type="text" placeholder="6-digit pincode" required />
                </div>
              </div>
            </div>
                ...
                <div className="flex gap-4 p-4 hairline border rounded-xl">
                  <div className="relative h-24 w-20 flex-shrink-0 overflow-hidden rounded-lg">
                    ...
                    <div
                      ...
                      style={{ backgroundColor: variant.colorHex, mixBlendMode: "multiply", opacity: 0.4 }}
                    />
                  </div>
